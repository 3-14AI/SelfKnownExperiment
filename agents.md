...
## Next Steps
- [x] Fixed flaky tests `test_is_sleeping`, `test_nocturnal_sleep_cycle`, `test_echolocation_night_perception`, `test_predator_adaptation`, `test_is_winter_strider_mutation`, `test_entity_perception_radius_food`, `test_sand_dancer_gains_energy_in_sandstorm`, and `test_is_web_strider_defense` and unskipped them to ensure robust test coverage.
...
- [x] Implemented `is_lava_dancer` trait. Entities with this trait gain energy when on `lava` terrain. (Fixed missing tests and mutation flakiness).
...
- [x] Implemented `is_storm_walker` trait. Entities with this trait do not consume extra stamina from elevation changes when moving during a 'storm' event.
...
- [x] Implemented `is_marsh_walker` trait. Entities with this trait do not consume extra stamina from elevation changes when moving on mud or water terrain.
...
- [x] Implemented `is_cave_dancer` trait. Entities with this trait gain energy when on `cave` terrain. Fully implemented mutation logic and energy gain in `engine.py` and comprehensive tests in `test_engine.py`.
...
- [x] Implemented `is_quicksand_dweller` trait. Entities with this trait treat `quicksand` terrain as a shelter, gaining increased defense and energy recovery. Rendered as 'õ' in CLI visualizer. Added missing engine and visualizer tests.
