    ...
    def move_entity(self, entity, dx, dy):
        ...
        if hasattr(entity, 'stamina'):
            ...
            if not getattr(entity, 'is_flying', False):
                if elevation_diff > 0:
                    if not getattr(entity, 'is_agile', False) and not getattr(entity, 'is_mountain_walker', False) and not (getattr(entity, 'is_cave_walker', False) and any(t.terrain_type == 'cave' for t in terrains_here)) and not (getattr(entity, 'is_snow_walker', False) and any(t.terrain_type == 'snow' for t in terrains_here)) and not (getattr(entity, 'is_blizzard_walker', False) and self.current_event == 'blizzard') and not (getattr(entity, 'is_storm_walker', False) and self.current_event == 'storm') and not (getattr(entity, 'is_rain_walker', False) and self.current_event == 'rain'):
