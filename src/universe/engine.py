...
class Universe:
    ...
    def find_path(self, start_x, start_y, target_x, target_y, radius=float('inf')):
            ...
            for dx, dy in directions:
                ...
                if (new_x, new_y) not in visited:
                    if 0 <= new_x < self.width and 0 <= new_y < self.height:
                        if abs(new_x - start_x) + abs(new_y - start_y) <= radius:
                            terrains_here = self.get_terrains_at(new_x, new_y)
    ...
    def get_nearest_food(self, x, y, radius=float('inf')):
        if not self.foods:
            return None
