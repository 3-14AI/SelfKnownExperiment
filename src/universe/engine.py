    ...
    def move_entity(self, entity, dx, dy):
        ...
        if hasattr(entity, 'stamina'):
            ...
            if getattr(entity, 'is_winter_glider', False) and self.current_season == 'winter':
                stamina_cost = 0
            if getattr(entity, 'is_rain_glider', False):
                for event in getattr(self, 'localized_events', []):
                    if event.event_type == 'rain' and (new_x - event.x)**2 + (new_y - event.y)**2 <= event.radius**2:
                        stamina_cost = 0
                        break
