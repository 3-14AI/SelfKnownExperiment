    ...
    def move_entity(self, entity, dx, dy):
        ...
        if hasattr(entity, 'stamina'):
            ...
            if getattr(entity, 'is_earthquake_glider', False) and self.current_event == 'earthquake':
                stamina_cost = 0
            in_rain = any(event.event_type == 'rain' and (new_x - event.x)**2 + (new_y - event.y)**2 <= event.radius**2 for event in self.localized_events)
            if getattr(entity, 'is_rain_glider', False) and in_rain:
                stamina_cost = 0
