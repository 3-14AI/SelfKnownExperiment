                    ...
                    for fy in range(max(0, event.y - event.radius), min(self.height, event.y + event.radius + 1)):
                        if (fx - event.x)**2 + (fy - event.y)**2 <= event.radius**2:
                            ...
                            for e in entities_here:
                                if getattr(e, 'is_volcanic', False) or getattr(e, 'is_fire_dweller', False):
                                    continue
                                e.energy = 0
        ...
        for entity in self.entities:
            ...
            if getattr(entity, 'is_hibernating', False):
                ...
                if getattr(entity, 'is_shelter_dweller', False) and any(t.terrain_type == 'shelter' for t in terrains_here):
                    energy_loss -= 2
                if getattr(entity, 'is_grass_dweller', False) and any(t.terrain_type == 'grass' for t in terrains_here):
                    energy_loss -= 2
                    ...
                    if foods_here:
                        ...
                        if preys_here:
                            ...
                            if getattr(prey_to_eat, 'is_shelter_dweller', False) and any(t.terrain_type == 'shelter' for t in self.get_terrains_at(prey_to_eat.x, prey_to_eat.y)):
                                effective_defense += 2
                            if getattr(prey_to_eat, 'is_grass_dweller', False) and any(t.terrain_type == 'grass' for t in self.get_terrains_at(prey_to_eat.x, prey_to_eat.y)):
                                effective_defense += 2
