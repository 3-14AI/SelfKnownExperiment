    ...
    def move_entity(self, entity, dx, dy):
        ...
        if hasattr(entity, 'stamina'):
            ...
            if getattr(entity, 'is_water_glider', False) and any(t.terrain_type == 'water' for t in terrains_here):
                stamina_cost = 0
            if getattr(entity, 'is_shelter_glider', False) and ((getattr(entity, 'is_blizzard_dweller', False) and self.current_event == 'blizzard') or (getattr(entity, 'is_storm_dweller', False) and self.current_event == 'storm') or any(t.terrain_type == 'shelter' for t in terrains_here)):
                stamina_cost = 0
