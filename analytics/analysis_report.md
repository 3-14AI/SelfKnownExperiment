...
- Updated `Universe.move_entity()` to reduce movement stamina cost to 0 when `entity.is_rain_glider` is True and the entity is within a 'rain' localized event radius.
...
- Added `TestIsRainGlider` test class in `tests/test_engine.py` to verify stamina conservation during rain events and correct mutation logic.
- Updated `agents.md` checklist with the completed trait.
