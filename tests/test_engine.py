...
class TestIsShelterDancerTrait(unittest.TestCase):
    ...
    @mock.patch('random.random')
    def test_is_shelter_dancer_mutation(self, mock_random):
        from src.universe.engine import Universe, Entity
        mock_random.return_value = 0.02
        universe = Universe(width=10, height=10, population_limit=100)
        parent = Entity(name="Parent", x=1, y=1, energy=50, size=1, lays_eggs=False, is_parasitic=False, is_vampiric=False, is_shelter_dancer=False)
        universe.add_entity(parent)
        universe.time = 0
        universe.tick()
        children = [e for e in universe.entities if getattr(e, 'generation', 0) == 1]
        self.assertTrue(len(children) > 0, "Reproduction failed")
        self.assertTrue(any(getattr(child, 'is_shelter_dancer', False) for child in children), "is_shelter_dancer failed to mutate")
